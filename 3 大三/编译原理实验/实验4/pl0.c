...
//F:\大三下\编译原理实验\实验4\monkeyNpeach.txt
void show_table(int tx){
	printf("\n\nTABLE:\n");
	...
	for(i=0;i<=tx;i++){
		...
		if(table[i].kind==constant){
			printf("const %7s   val=%2d   addr=%2d\n",
				table[i].name,table[i].val,table[i].adr);
		}
		else if(table[i].kind==variable){
			printf("var   %7s   lev=%2d   addr=%2d\n"
				,table[i].name,table[i].level,table[i].adr);
		}
		else if(table[i].kind==procedur){
			printf("proc  %7s   lev=%2d   addr=%2d  size=%2d\n"
				,table[i].name,table[i].level,table[i].adr,table[i].size);
		}
		else{
			printf("none \n");
		}
...
int block(int lev, int tx, bool* fsys)
{
	...
	dx = 3;                 // 符号表初始状态的偏移量，因为前三个要关联变量 
	tx0 = tx;               // 记录本层名字的初始位置，主函数里等于0 
	table[tx].adr = cx;     //符号表的第0个位置为当前执行的语句位置 
	...
	do {
		...
		while (sym == procsym) /* 收到过程声明符号，开始处理过程声明 */
		{
			...
			if(sym == semicolon)
			{
				...
				nxtlev[ident] = true;
				nxtlev[procsym] = true;
				//zjx：这里是作为补救，如果詈蠹由的范围要大于fsys，
				//所以我认为下面的式子写反了，不过无伤大雅
